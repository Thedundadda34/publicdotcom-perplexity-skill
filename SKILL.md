---
name: publicdotcom-perplexity-skill
description: Interact with your Public.com brokerage account using the Public.com API in Perplexity Computer. Able to view portfolio, get stock quotes, place trades, and get account updates. To create a Public.com account head to public.com/signup.
license: Apache-2.0
metadata:
  author: PublicDotCom
  source: https://github.com/PublicDotCom/publicdotcom-perplexity-skill
  category: "Finance"
...
**Workflow:**
1. If the user specifies an expiration date, go directly to step 2. If they say something like "options expiring around earnings" or don't specify a date, use `get_option_expirations.py [SYMBOL]` to retrieve available dates, present them to the user, and ask them to confirm which one to use (or choose the nearest/most relevant one based on context).
...
- `--time-in-force`: DAY (default) or GTC (Good Till Cancelled)
- `--account-id`: Account ID (uses `PUBLIC_COM_ACCOUNT_ID` env var if not provided)
